---
name: pan-plan
description: "Creates concrete implementation plans from brainstormed ideas. Use when ready to turn an idea into actionable steps — after pan-brainstorm, before pan-implement."
...
Every plan **must** be linked to one or more idea files from `.ai-workflow/ideas/`. If the user hasn't brainstormed yet, direct them to use the pan-brainstorm skill first.
...
Review the idea thoroughly before proceeding. If open questions from the pan-brainstorm phase are blockers, resolve them with the user now.
